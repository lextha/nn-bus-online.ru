...
if (count($city_s)<4) { $city_s=[$city->name,$city->name,$city->name,$city->name,$city->name,$city->name]; }
 //var_dump($city_s); die();
...
function vydel($text){
    ...
    $len= intdiv(count($array),4);// ограничитель кол-ва вывода времени при первой загрузке
   /* if ($_SERVER['REMOTE_ADDR']=='5.187.69.150') {
        var_dump(count($array),$len);
    }*/
    $lenlen=1;
    if ($len==0) {
        $len=1;
    ...
    foreach ($array as $key => $value) {
        if ($len>0) {
            if ($len==1) { $wfgf=" style='-webkit-mask-image: -webkit-gradient(linear, left top, right bottom, from(rgba(0,0,0,1)), to(rgba(0,0,0,0)));'"; } else { $wfgf=''; }
            $value= str_replace("++", " ", $value);
            if (preg_match("/(\d+)[:-](\d+)(.*)$/", $value)) {
                if ($flag) {
                    $arr.="</span>";
                }
                $arr.=" <span".$wfgf.">".$value."</span>";
                $flag=false;
            } else {
                if ($flag) {
                    $arr.=$value;
    ...
    if (isset($ttl[1]) AND $lenlen==0) {
        $ttl[1] = mb_substr($ttl[1], 0, 20);
        $arr.="<span>".$ttl[1]."...</span>";
    }
    return $arr;
}
 ...
 $html='';
    foreach ($marsh as $in) { 
      ...
      //  $html.='>';
        ...
        $ds='';
        $ds.="<span class='getmarsh'>Показать время</span> "; 
        
       if (count($time)<2) {
           $html.='<div class="level">'.$in[0].'</div>';
           if ($time[0]!='') { $html.='<div class="hide show bud_t value"><span class="title_time">Будни</span>'.$time[0].'</div>'.$ds; }
       }
       elseif (count($time)==2) {
           $html.='<div class="level">'.$in[0].'</div><div class="hide show bud_t value"><span class="title_time">Будни</span>'.$time[0].'</div><div class="hide show vyh_t value" style="display:none;"><span class="title_time">Выходные</span>'.$time[1].'</div>'.$ds;
       }
       elseif (count($time)>2) {
...
/*if ($_SERVER['REMOTE_ADDR']=='5.187.70.113') {
   // var_dump($marshrut_in,$html_out);
    echo $marshrut_in[0][0]." - ".$marshrut_in[array_key_last($marshrut_in)][0];
}*/
 /////////////////////
...
/*?>
	...
	</div><!-- .route-top route3 -->

               

                <div class="route-stop" id="route-stop" idi="<?=$route->id;?>" <? if (!((!$route->active) OR (mb_strlen($route->time_work)>5 OR mb_strlen($route->info)>4) OR (mb_strlen($route->route_text)>5))) { echo "style='display:none;'"; } ?> >
			...
			<? /* <h2 class="subtitle">Остановки и время отправления <?=$type_transport2;?></h2> */ ?>
                        ...
                        <? if (mb_strlen($route->time_work)>5 OR mb_strlen($route->info)>4) {  ?>
                        <div class="item item-2">
                            <div class="level">Расписание и интервал движения:</div>
                            <div class="value"><?

                            if (mb_strlen($route->time_work)>5) {
                                $str=nl2br($route->time_work);
                            ...
                            if (mb_strlen($route->info)>4) { echo nl2br($route->info); }?>

                            </div>
                        </div>
                        <? } ?>    

                         <? if (mb_strlen($route->route_text)>5) {  ?>
                            <div class="item item-4">
                                <div class="level">Маршрут движения:</div>
                                <div class="value"><?=nl2br($route->route_text)?></div>
                            </div>
                        <? }?>    
...
echo $html_out?>
			  ...
			  <? //if ($_SERVER['REMOTE_ADDR']=='5.187.78.115') { 
                                        if (count($marshrut_in)>0) {
                                            echo '<div class="route-stop"><div class="item item-3"><div class="level">Дополнительная информация:</div><div class="value">';
                                           if ($route->type_direction==3) {
                                           ...
                                           } elseif ($route->season==2) {
                                               echo "<p>Садовый (дачный) маршрут ".$title_str." выполняет рейсы по расписанию. Уточняйте даты начала и окончания действия у перевозчика."; 
                                           }elseif ($route->season==1) {
                                               echo "<p>Сезонный маршрут ".$title_str." выполняет рейсы по расписанию. Уточняйте даты начала и окончания действия у перевозчика."; 
                                           }elseif ($route->type_direction==2) {
                                               echo "<p>Пригородный маршрут ".$title_str." выполняет рейсы по расписанию. Уточняйте сезонность перевозок этого маршрута."; 
                                           } else {
                                               $trans_predpr='';
                                               if (strlen($route->organization)>3) { $trans_predpr='Обслуживает это направление '.str_replace("\\", "", $route->organization).". ";}
...
<? ////////////////////////////////////////////////////////////////////////////////?>
    ...
    <div style="display:none;"><noindex><!--googleoff: all--><div class="modalwin" id="errorfindmodal"></div> <!--googleon: all--></noindex></div><div style="display:none;"><noindex><!--googleoff: all-->2011 © на базу данных Go on Bus . ru <? echo "route3 new"; ?></noindex></div>
</div>
</div>
</div>
<? if (Yii::$app->user->can('admin')) { echo "route3 new"; } ?>